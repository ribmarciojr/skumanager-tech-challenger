...
skuRouter.post("/sku", async (req, res, next) => {
  ...
  return adaptController((req, res) => createSkuController.create(req, res))(
    req,
    res,
    next,
  );
...
skuRouter.get("/sku/all", async (req, res, next) => {
  ...
  return adaptController((req, res) => listSkuController.listSkus(req, res))(
    req,
    res,
    next,
  );
...
skuRouter.get("/sku/status", async (req, res, next) => {
  ...
  await adaptController((req, res) =>
    listSkuController.listSkusStatusValues(req, res),
  )(req, res, next);
});

skuRouter.get("/sku/:id", async (req, res) => {
  const getSkuByIdUseCase = new GetSkuByIdUseCase(skuRepository, winstonLogger);
  const getSkuByIdController = new GetSkuByIdController(
    getSkuByIdUseCase,
    winstonLogger,
  );
...
skuRouter.put("/sku/:id/complete", async (req, res, next) => {
  ...
  const completeSkuController = new CompleteSkuRegistrationController(
    completeSkuUseCase,
  );

  return adaptController((req, res) =>
    completeSkuController.complete(req, res),
  )(req, res, next);
});
...
skuRouter.put("/sku/:id", async (req, res, next) => {
  ...
  return adaptController((req, res) => updateSkuController.update(req, res))(
    req,
    res,
    next,
  );
...
skuRouter.get("/health", async (req, res, next) => {
  try {
    res.status(200).json({
      status: "Healthyt7r85tde",
      timestamp: new Date().toISOString(),
      service: "sku-management-api",
    });
  } catch (error) {
    next(error);
  }
