...
export class GetSkuByIdController {
  ...
  async get(req: Request, res: Response) {
    ...
    if (typeof rawSkuId !== "string") {
      throw new ValidationException("SkuId", "SkuId must be string.");
    }
    ...
    this.getSkuByIdUseCase
      ...
      .then((sku) => {
        ...
        if (!skuCurrentStatus) {
          throw new NotFoundException(
            `Falha ao encontrar status do sku ${sku.status?.status}`,
          );
        }

        const stateMachine = new SkuStateMachine(
          SkuStatusMapper.mapFromKey(skuCurrentStatus),
        );
      ...
      .catch((error: unknown) => {
        if (error instanceof NotFoundException) {
          ...
          this.logger.error(
            "Erro desconhecido ao buscar SKU.",
